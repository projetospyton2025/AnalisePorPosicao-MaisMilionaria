"""
Serviço para integração com a API da Caixa - Dupla Sena
"""
import requests
...
class ApiDuplaSenaService:
    """Serviço para buscar dados da API oficial da Caixa - Dupla Sena"""
    ...
    def buscar_ultimo_concurso(self) -> Optional[Dict]:
        ...
        try:
            ...
            print(f"Erro ao buscar último concurso Dupla Sena: {e}")
            return None
    ...
    def buscar_concurso(self, numero: int) -> Optional[Dict]:
        ...
        try:
            ...
            print(f"Erro ao buscar concurso Dupla Sena {numero}: {e}")
            return None
    ...
    def atualizar_base_dados(self, concurso_inicial: int = 1) -> Dict:
        ...
        if not ultimo:
            return {
                'sucesso': False,
                'mensagem': 'Não foi possível buscar o último concurso da Dupla Sena',
                'inseridos': 0,
                'erros': 0
            }
        ...
        return {
            'sucesso': True,
            'mensagem': f'Atualização Dupla Sena concluída: {inseridos} concursos inseridos',
            'inseridos': inseridos,
            'erros': erros,
