"""
Model para armazenamento de resultados da +Milionária
"""
import sqlite3
import json
...
class ResultadoModel:
    """Model para gerenciar resultados da +Milionária"""
    ...
    def _criar_tabela(self):
        ...
        cursor.execute('''
            CREATE TABLE IF NOT EXISTS resultados (
                ...
                trevos_sorteados TEXT NOT NULL,
    ...
    def inserir_resultado(self, resultado: Dict) -> bool:
        ...
        try:
            ...
            trevos_sorteados = json.dumps(resultado.get('trevosSorteados', []))
            ...
            cursor.execute('''
                INSERT OR REPLACE INTO resultados (
                    numero, data_apuracao, acumulado, lista_dezenas,
                    trevos_sorteados, dezenas_ordem_sorteio, valor_arrecadado,
                    valor_acumulado_proximo_concurso, valor_estimado_proximo_concurso,
                    lista_rateio_premio, lista_municipio_uf_ganhadores, observacao,
                    ultimo_concurso
                ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            ''', (
    ...
    def _row_to_dict(self, row) -> Dict:
        ...
        return {
            ...
            'listaDezenas': json.loads(row[3]),
            'trevosSorteados': json.loads(row[4]),
            'dezenasSorteadasOrdemSorteio': json.loads(row[5]),
            'valorArrecadado': row[6],
            'valorAcumuladoProximoConcurso': row[7],
            'valorEstimadoProximoConcurso': row[8],
            'listaRateioPremio': json.loads(row[9]) if row[9] else [],
            'listaMunicipioUFGanhadores': json.loads(row[10]) if row[10] else [],
            'observacao': row[11],
            'ultimoConcurso': bool(row[12])
        }
